...
static const char* libstocheck[NUM_LIBS] = { "libovrplatformloader.so", LIBC };
        ...
        if (buf[0] == 'r') {
            ...
            for (int i = 0; i < pElfSectArr->execSectionCount; i++) {
                ...
                else
                {
                    ...
                    //__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Checksum:[%ld][%ld]", output, pElfSectArr->checksum[i]);
            ...
            for (int i = 0; i < NUM_LIBS; i++) {
                if (my_strstr(map, libstocheck[i]) != NULL) {
                    if (true == scan_executable_segments(map, elfSectionArr[i], libstocheck[i])) {
                        // Return true means libstocheck[i] has been manipulated, do Anti-crack here.
                        break;
                    }
                }
            }
